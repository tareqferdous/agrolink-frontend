      ...
      {value.length > 0 && (
        <div className='grid grid-cols-3 gap-2 mb-3'>
          {value.map((url, index) => (
            <div
              key={url}
              className='relative aspect-square rounded-xl overflow-hidden group border border-gray-100 dark:border-gray-700'>
              <Image
                src={url}
                alt={`Image ${index + 1}`}
                fill
      ...
      {value.length < maxImages && (
        <button
          ...
          className={`w-full py-8 border-2 border-dashed rounded-xl text-sm transition-colors flex flex-col items-center gap-2 ${
            uploading
              ? "border-green-300 dark:border-green-700 bg-green-50 dark:bg-green-900/30 text-green-600 dark:text-green-300 cursor-wait"
              : "border-gray-200 dark:border-gray-700 text-gray-400 dark:text-gray-500 hover:border-green-300 dark:hover:border-green-700 hover:bg-green-50 dark:hover:bg-green-900/30 hover:text-green-600 dark:hover:text-green-300 cursor-pointer"
          }`}>
          {uploading ? (
            <>
